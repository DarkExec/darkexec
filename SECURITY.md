...
The editable standard and efficiency harness prompts are independently bounded, atomically replaced, and stored in mode-0600 runtime files beneath a mode-0700 state directory. Project overrides accept only exact saved Codex projects, use hashed private storage identities, and fall back to the host-wide prompt without exposing arbitrary filesystem access. The read-only fire-drill prompt is fixed separately and cannot be changed through either setting.
