...
`models --json` exposes the provider-qualified catalog and each model's valid thinking and speed controls. `dispatch`, `run`, and `continue` accept `--model`, `--thinking-level low|medium|high|xhigh|max|ultra`, and `--speed standard|fast`. Codex models run through the App Server subscription transport. Gemini 3.8 Flash dispatches and continues through the signed-in Antigravity CLI subscription with `accept-edits` and `--dangerously-skip-permissions`; Low, Medium, and High select the corresponding native Gemini variant. The executive-only `run` path remains Codex-native. Omission preserves native settings, while `standard` explicitly clears Codex Fast mode. Same-turn steering cannot change execution options; use them on the next continuation. Dispatch deduplication includes the selected options.

`set-execution-options --target PATH --thread ID --model PROVIDER/MODEL --thinking-level LEVEL --speed standard|fast --json` saves an exact session override without starting or interrupting a turn. Model and thinking changes within the current provider apply to the next turn on the same native conversation. Crossing between Codex and Antigravity requires a new assignment because their native context formats are not interchangeable. Every later target turn, including closeout and calls from workers started before an App upgrade, reads the saved override.
