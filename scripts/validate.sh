...
grep -Fq 'PR #30 regression: moving harness passes into a fresh task' "$root/ARCHITECTURE.md"
grep -Fq 'Keep Markdown prose and list items on one physical line; never hard-wrap.' "$root/AGENTS.md"
...
grep -Fq 'managed_doctrine="$doctrine_root/current/harness-ops.md"' "$root/scripts/install.sh"
grep -Fq 'ln -sfn "$managed_doctrine" "$host_doctrine"' "$root/scripts/install.sh"
grep -Fqx 'TURN_TIMEOUT = int(os.environ.get("DARKEXEC_TURN_TIMEOUT", "0"))' "$root/bin/darkexec"
grep -Fq 'docs/passes/harness/AGENTS.md' "$root/bin/darkexec"
grep -Fq 'docs/passes/efficiency/AGENTS.md' "$root/bin/darkexec"
