...
grep -Fqx '4. Preserve the request byte-for-byte as the target task'"'"'s first real turn and wait directly. With `darkexec run`, pipe it with `printf '"'"'%s'"'"'`, never `printf '"'"'%s\n'"'"'`. Invoke that synchronous command once with the longest available tool timeout and keep its process alive until terminal JSON arrives; never poll by timing it out and reissuing it. Treat a caller-side timeout or interruption as terminal for this executive task and report it without retrying. Treat complete terminal JSON as identity and status proof; reread only if that result is contradictory.' "$root/share/workspace/AGENTS.md"
