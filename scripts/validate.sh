...
grep -Fqx '4. On the first delegated request, create one target task, preserve the request byte-for-byte as its first real turn, and wait directly. With `darkexec run`, pipe it with `printf '"'"'%s'"'"'`, never `printf '"'"'%s\n'"'"'`, invoke it once with the longest available timeout, and keep it alive through its immediate same-task harness and terminal JSON. With native controls, immediately send the matching harness prompt from step 5 to that target and wait before reporting. Never start multiple synchronous DarkExec runs inside one finite caller deadline; give each run a complete owned budget or use independently owned background dispatch. Treat any caller timeout or interruption as terminal and never retry or resume automatically.' "$root/share/workspace/AGENTS.md"
