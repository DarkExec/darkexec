...
grep -Fqx '3. Create one App task with the request as its first turn. Use native task tools when available; otherwise pipe the exact request to `darkexec run --target PATH --prompt-stdin [--read-only-harness] --json`. Never reconstruct App Server protocol by hand.' "$root/share/workspace/AGENTS.md"
